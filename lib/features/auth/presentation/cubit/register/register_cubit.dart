...
class RegisterCubit extends Cubit<RegisterState> {
  ...
  RegisterCubit(this._authRepo, this._imagePickerService)
  ...
  Future<void> getCountryLocations() async {
    ...
    result.when(
      onSuccess: (response) => emit(
        state.copyWith(getLocationsState: AsyncData(response.data ?? [])),
      ),
      onFailure: (error) =>
          emit(state.copyWith(getLocationsState: AsyncError(error))),
    );
  }
  ...
  Future<void> getReceivingBranches() async {
    ...
    result.when(
      onSuccess: (branches) =>
          emit(state.copyWith(getBranchesState: AsyncData(branches))),
      onFailure: (error) =>
          emit(state.copyWith(getBranchesState: AsyncError(error))),
    );
  }
  ...
  Future<void> register() async {
    if (state.pickedImage == null) {
      emit(
        state.copyWith(
          registerState: AsyncError(
            ApiErrorModel(error: 'please_select_id_photo'),
          ),
        ),
      );
      return;
    ...
    result.when(
      onSuccess: (message) =>
          emit(state.copyWith(registerState: AsyncData(message))),
      onFailure: (error) =>
          emit(state.copyWith(registerState: AsyncError(error))),
    );
  }
  ...
  void updateWhatsappKey(String value) =>
      emit(state.copyWith(whatsappKey: value));
  ...
  void selectCity(CityModel? city) {
    emit(state.copyWith(selectedCity: city, selectedRegion: null));
  }
