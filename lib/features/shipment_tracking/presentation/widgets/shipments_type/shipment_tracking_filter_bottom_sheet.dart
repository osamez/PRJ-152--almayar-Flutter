...
class ShipmentTrackingFilterBottomSheet extends StatefulWidget {
  const ShipmentTrackingFilterBottomSheet({super.key});

  static void show(BuildContext context) {
    final cubit = context.read<ShipmentTrackingCubit>();
    showModalBottomSheet(
      ...
      builder: (_) => BlocProvider.value(
        value: cubit,
        child: const ShipmentTrackingFilterBottomSheet(),
      ),
    );
  }

  @override
  State<ShipmentTrackingFilterBottomSheet> createState() =>
      _ShipmentTrackingFilterBottomSheetState();
}

class _ShipmentTrackingFilterBottomSheetState
    extends State<ShipmentTrackingFilterBottomSheet> {
  int? _selectedIndex;

  @override
  void initState() {
    super.initState();
    final state = context.read<ShipmentTrackingCubit>().state;
    ...
    return BlocBuilder<ShipmentTrackingCubit, ShipmentTrackingState>(
      builder: (context, state) {
        final options = _getFilterOptions(
          state.selectedTabIndex == 0 ? ShipmentType.air : ShipmentType.sea,
        );
        return Padding(
            ...
            children: [
              ...
              AppElevatedButton(
                ...
                onPressed: () {
                  ...
                  context.read<ShipmentTrackingCubit>().applyFlightTypeFilter(
                    flightType,
                  );
